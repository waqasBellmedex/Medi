...
from inserted as i
inner join deleted as d on i.ID = d.ID and ISNULL(i.BillerID,'1') != ISNULL(d.BillerID,'1')
Join Biller as cDel on cDel.ID = ISNULL(d.BillerID,'1')
join Biller as cUpd on cUpd.ID = ISNULL(i.BillerID,'1')	
END TRY  
BEGIN CATCH  
    --SELECT   
     print ERROR_NUMBER(); --AS ErrorNumber  
...
insert into BatchDocumentAudit(BatchDocumentID,TransactionID,ColumnName,CurrentValue,NewValue,HostName,AddedBy,AddedDate)
select i.ID, @TransactionID,'Admit Notes',(RTRIM(LTRIM(d.AdmitNotes))),(RTRIM(LTRIM(i.AdmitNotes))),HOST_NAME(),i.UpdatedBy ,CURRENT_TIMESTAMP
from inserted as i
inner join deleted as d on i.ID = d.ID and ISNULL(RTRIM(LTRIM(i.AdmitNotes)),'') != ISNULL(RTRIM(LTRIM(d.AdmitNotes)),'')
END TRY  
BEGIN CATCH  
    --SELECT   
     print ERROR_NUMBER(); --AS ErrorNumber  
...
insert into BatchDocumentAudit(BatchDocumentID,TransactionID,ColumnName,CurrentValue,NewValue,HostName,AddedBy,AddedDate)
select i.ID, @TransactionID,'Payment Notes',(RTRIM(LTRIM(d.PaymentNotes))),(RTRIM(LTRIM(i.PaymentNotes))),HOST_NAME(),i.UpdatedBy ,CURRENT_TIMESTAMP
from inserted as i
inner join deleted as d on i.ID = d.ID and ISNULL(RTRIM(LTRIM(i.PaymentNotes)),'') != ISNULL(RTRIM(LTRIM(d.PaymentNotes)),'')
END TRY  
BEGIN CATCH  
    --SELECT   
     print ERROR_NUMBER(); --AS ErrorNumber  
